  ...
  const [errors,seterrors] = useState('');
    ...
    const fetchWorkers = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5000/api/getelectrician/${adminId}`);
    ...
    const handleAddWorker = async () => {
      try {
        const response = await axios.post("http://localhost:5000/api/addelectrician", newWorker);
    ...
    const handleDeleteWorker = async (id) => {
      try {
        await axios.delete(`http://localhost:5000/api/deleteelectrician/${id}`);
    ...
    <div className="admin-page">
      ...
      <div className="list-container">
        ...
        {workers.length > 0 ? (
              ...
              {workers.map((worker, index) => (
                <tr key={index}>
                  ...
                  <td>
                    ...
                    <button onClick={() => handleDeleteWorker(index)} className="delete-btn">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <p className="admin-no-workers">No electricians available.</p>
        )}
      </div>
