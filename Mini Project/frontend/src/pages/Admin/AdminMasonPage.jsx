...
const AdminMasonPage = () => {
  const user = useSelector((state) => state.auth.user);
    ...
    const fetchWorkers = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5000/api/getmason/${adminId}`);
    ...
    const handleAddWorker = async () => {
      try {
        const response = await axios.post("http://localhost:5000/api/addmason", newWorker);
    ...
    const handleDeleteWorker = async (id) => {
      try {
        await axios.delete(`http://localhost:5000/api/deletemason/${id}`);
  ...
  return (
    <div className="admin-page">
      ...
      <img className="img" src={MasonI} alt="Mason" />
    </div>
  );
};
