...
const submitRating = async (workerId) => {
  ...
  try {
    await axios.post(`http://localhost:5000/api/ratecarpenter/${workerId}`, { rating: selectedRating });
...
const handlePayment = async (carpenter) => {
  ...
  const { data } = await axios.post("http://localhost:5000/api/create-order" , {amount : enteredAmount});
  ...
  const options = {
    ...
    handler: async function (response) {
      ...
      await axios.post("http://localhost:5000/api/save-payment", {
        adminId : carpenter.adminId,
        payment_id: response.razorpay_payment_id,
        order_id: response.razorpay_order_id,
        signature: response.razorpay_signature,
        amount: data.amount / 100,
        workerId: carpenter._id,
        method: "razorpay",
      });
    },
    theme: {
         ...
         <div style={{position:'absolute', top:'200px', left:'750px'}}className="worker-card">
           <div className="worker-card-content">
             ...
             <button style={{display:'block',position:'relative',left:'10px'}} onClick={() => window.location.href = `tel:${selectWorker.contact}`} 
              ...
              <button style={{display:'block',position:'absolute',right:'60px'}}className="contact-btn" onClick={handlePayment(selectWorker)}>Pay</button>;
